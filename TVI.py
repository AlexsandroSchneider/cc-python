...
def intervalo(x, y):
    if (fx(x)>=0 and fx(y)>=0) or (fx(x)<0 and fx(y)<0):
        return "invalido"
    return
...
def menu():
    while True:
        try:
            ...
            if intervalo(x, y)=="invalido": #caso n1 e n2 sejam ambos >= 0 ou ambos < 0
                raise Exception
            break
    ...
    if x > y:
        ...
        menor = x
        maior = y
    print(f"Intervalo: x={menor}, f({menor})= {fx(menor):.4f} e x={maior}, f({maior})= {fx(maior):.4f}")
